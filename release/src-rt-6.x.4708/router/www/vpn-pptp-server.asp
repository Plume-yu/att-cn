...
<content>
	<script type="text/javascript">
		...
		ul.dataToView = function(data) {
			return [data[0], '<small><i>秘密</i></small>'];
		}
		function save() {
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="pptp-server">
			<div class="heading">PPTP 服务器</div>
			<div class="content">
				...
				<script type="text/javascript">
					$('#pptp-server-settings').forms([
						...
						{ title: '本地IP地址/子网掩码', text: (nvram.lan_ipaddr + ' / ' + nvram.lan_netmask) },
						{ title: '客户端IP地址池', multi: [
						...
						{ title: '广播中继模式', name: 'pptpd_broadcast', type: 'select',
							options: [['disable','禁用'], ['br0','LAN到VPN客户端'], ['ppp','VPN客户端到LAN'], ['br0ppp','两者']],
							value: nvram.pptpd_broadcast, suffix: ' <span style="color: red; font-size: 80%">启用此功能可能会导致CPU使用率过高</span>' },
						{ title: '加密模式', name: 'pptpd_forcemppe', type: 'select', options: [[0, '无'], [1, 'MPPE-128']], value: nvram.pptpd_forcemppe },
						{ title: 'DNS服务器', name: 'pptpd_dns1', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_dns1 },
						{ title: '', name: 'pptpd_dns2', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_dns2 },
						{ title: 'WINS服务器', name: 'pptpd_wins1', type: 'text', maxlen: 15, size: 17, value: nvram.pptpd_wins1 },
						...
						{ title: '自定义iptables if-up规则', name: 'pptpd_ipup_script', type: 'textarea', value: nvram.pptpd_ipup_script, style: 'width: 100%; height: 80px;' },
						{ title: '自定义iptables if-down规则', name: 'pptpd_ipdown_script', type: 'textarea', value: nvram.pptpd_ipdown_script, style: 'width: 100%; height: 80px;' }
					]);
				</script>
				...
				<div class="section" id="sesdiv_notes" style="display:none">
					<ul>
						<li><b>本地IP / 子网掩码</b> - 用于在服务器和VPN客户机之间建立PPTP隧道链接.</li>
						<li><b>客户端IP地址池</b> - 用于为 VPN客户机分配 IP. (最多允许6个IP).</li>
						<li><b>广播中继模式</b> - 允许 VPN客户端 和 本地 LAN 之间通讯.</li>
						<li><b>启用加密</b> - 启用该选项后将对VPN通道进行加密以提升安全性，但是会导致VPN通道宽带减少.</li>
						<li><b>DNS 服务器</b> - 使用自定义 DNS服务器 (如果未设置，VPN客户端将使用路由器的本地IP地址).</li>
						<li><b>WINS 服务器</b> - 设置 WINS服务器IP, 也可以在 <a href=#basic-network.asp>基本设置 - 网络设置</a>中进行设置.</li>
					...
					<ul>
						<li><b>其他相关的说明/提示:</b>
							<ul>
								<li>应该避免让本地DHCP地址和VPN客户机IP地址重叠</li>
							</ul>
						</li>
					</ul>
		...
		<a class="pull-right btn" href="/#vpn-pptp-online.asp">&raquo; PPTP在线用户</a>
