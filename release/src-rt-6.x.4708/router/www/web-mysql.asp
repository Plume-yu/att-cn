	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="mysql-basic">
			...
			<div class="content">
				...
				<hr>
				<ul>
					<li><b>启用 MySQL服务器</b> - 警告！ - 如果你的路由器只有32MB的RAM，你必须使用交换。
					<li><b>系统目录</b> - 包含mysqld等的目录的路径不包括程序名“/ mysqld”
					<li><b>进程守护</b> - 开启后，Mysql将按一定间隔检查进程，如果发现程序崩溃就尝试重新启动服务。
					<li><b>数据和tmp目录</b> - 注意! 不能使用NAND分区做数据和tmp目录
				</ul>
			</div>
		</div>
		...
		<script type="text/javascript">
			...
			$('#mysql-basic-set').forms([
				{ title: '启用 MySQL服务器启用 MySQL服务器', name: 'f_mysql_enable', type: 'checkbox', value: nvram.mysql_enable == 1, suffix: ' <small>*</small>' },
				{ title: '系统目录', multi: [
					{ name: 'mysql_binary', type: 'select', options: [
						['internal','内部 (/usr/bin)'],
						['optware','Optware (/opt/bin)'],
						['custom','自定义'] ], value: nvram.mysql_binary, suffix: ' <small>*</small> ' },
					{ name: 'mysql_binary_custom', type: 'text', maxlen: 40, size: 40, value: nvram.mysql_binary_custom , suffix: ' <small>不包含 "/mysqld"</small>' }
				] },
				{ title: '进程守护', name: 'f_mysql_check', type: 'checkbox', value: nvram.mysql_check == 1, suffix: ' <small>*</small>' },
				{ title: '进程守护间隔', indent: 2, name: 'mysql_check_time', type: 'text', maxlen: 5, size: 7, value: nvram.mysql_check_time, suffix: ' <small>分钟 (范围: 1 - 55; 默认: 1)</small>' },
				{ title: '启动延迟', name: 'mysql_sleep', type: 'text', maxlen: 5, size: 7, value: nvram.mysql_sleep, suffix: ' <small>秒 (范围: 1 - 60; 默认: 2)</small>' },
				{ title: 'MySQL监听端口', name: 'mysql_port', type: 'text', maxlen: 5, size: 7, value: nvram.mysql_port, suffix: ' <small> 默认: 3306</small>' },
				{ title: '允许任何主机访问', name: 'f_mysql_allow_anyhost', type: 'checkbox', value: nvram.mysql_allow_anyhost == 1, suffix: ' <small>允许任何主机访问数据库服务器。</small>' },
				{ title: '重新初始化数据库', name: 'f_mysql_init_priv', type: 'checkbox', value: nvram.mysql_init_priv== 1, suffix: ' <small>如果选中，特权表将被强制通过mysql_install_db重新初始化。</small>' },
				{ title: '重新初始化ROOT密码', name: 'f_mysql_init_rootpass', type: 'checkbox', value: nvram.mysql_init_rootpass == 1, suffix: ' <small>如果选中，root密码将被强制重新初始化。</small>' },
				{ title: 'ROOT账号', name: 'mysql_username', type: 'text', maxlen: 32, size: 16, value: nvram.mysql_username, suffix: ' <small>连接到服务器的用户名（默认：root）</small>' },
				{ title: 'ROOT密码', name: 'mysql_passwd', type: 'password', maxlen: 32, size: 16, peekaboo: 1, value: nvram.mysql_passwd, suffix: ' <small>不允许空。（默认值：admin）</small>' },
				{ title: '启用USB分区', multi: [
				...
				{ title: '缓存目录（tmp）.', indent: 2, name: 'mysql_tmpdir', type: 'text', maxlen: 50, size: 40, value: nvram.mysql_tmpdir, suffix: ' <small>已挂载分区下的目录名称</small>' }
			]);
		</script>
		...
		<div class="box" data-box="mysql-advanced">
			...
			<div class="content">
				...
				<hr>
				<ul>
					<li><b>MySQL服务器自定义配置</b> -设置方法:  param=value   如：  connect_timeout=10</li>
				</ul>
			</div>
		</div>
		...
		<script type="text/javascript">
			$('#mysql-advanced-set').forms([
				{ title: 'Key buffer', name: 'mysql_key_buffer', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_key_buffer, suffix: ' <small>MB (范围: 1 - 1024; 默认: 8)</small>' },
				{ title: 'Max allowed packet', name: 'mysql_max_allowed_packet', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_max_allowed_packet, suffix: ' <small>MB (范围: 1 - 1024; 默认: 4)</small>' },
				{ title: 'Thread stack', name: 'mysql_thread_stack', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_thread_stack, suffix: ' <small>KB (范围: 1 - 1024000; 默认: 192)</small>' },
				{ title: 'Thread cache size', name: 'mysql_thread_cache_size', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_thread_cache_size, suffix: ' <small>(范围: 1 - 999999; 默认: 8)</small>' },
				{ title: 'Table open cache', name: 'mysql_table_open_cache', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_table_open_cache, suffix: ' <small>(范围: 1 - 999999; 默认: 4)</small>' },
				{ title: 'Query cache size', name: 'mysql_query_cache_size', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_query_cache_size, suffix: ' <small>MB (范围: 0 - 1024; 默认: 16)</small>' },
				{ title: 'Sort buffer size', name: 'mysql_sort_buffer_size', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_sort_buffer_size, suffix: ' <small>KB (范围: 0 - 1024000; 默认: 128)</small>' },
				{ title: 'Read buffer size', name: 'mysql_read_buffer_size', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_read_buffer_size, suffix: ' <small>KB (范围: 0 - 1024000; 默认: 128)</small>' },
				{ title: 'Read rand buffer size', name: 'mysql_read_rnd_buffer_size', type: 'text', maxlen: 10, size: 10, value: nvram.mysql_read_rnd_buffer_size, suffix: ' <small>KB (范围: 1 - 1024000; 默认: 256)</small>' },
				...
				{ title: 'MySQL服务器自定义配置', name: 'mysql_server_custom', type: 'textarea', value: nvram.mysql_server_custom, style: 'width: 100%; height: 80px;' }
			]);
		</script>
