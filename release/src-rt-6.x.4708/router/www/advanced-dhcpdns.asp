		...
		<div class="box" data-box="dhcp-server">
			...
			<script type="text/javascript">
				$('.section.dnsdhcp').forms([
					...
					{ title: '阻止DNS-rebind攻击', name: 'f_dns_norebind', type: 'checkbox', value: nvram.dns_norebind == '1' },
					{ title: '拦截DNS端口', name: 'f_dns_intcpt', type: 'checkbox', value: nvram.dns_intcpt == '1' },
					...
					{ title: '广播IPv(SLAAC)', name: 'f_ipv6_radvd', type: 'checkbox', value: nvram.ipv6_radvd == '1' },
					{ title: '广播IPv6 (DHCP)', name: 'f_ipv6_dhcpd', type: 'checkbox', value: nvram.ipv6_dhcpd == '1' },
					{ title: '隐藏DHCPV4记录g', name: 'f_dnsmasq_q4', type: 'checkbox', value: (nvram.dnsmasq_q & 1) },
		...
		<div class="box">
			...
			<div class="section content" id="sesdivnotes" style="display:none">
				...
				<ul>
					...
					<li><b>阻止DNS-rebind攻击</b> - 在Dnsmasq上启用DNS污染保护。</li>
					<li><b>拦截DNS端口</b> - 任何发送到UDP/TCP端口53的DNS请求/数据包都将重定向到内部DNS服务。目前仅拦截IPv4的DNS。</li>
					<li><b>WAN禁用则使用自定义网关</b> - DHCP将使用路由器的IP地址作为每个LAN上的默认网关。</li>
					<li><b>忽略DHCP请求 (...)</b> - 在 <a href='#basic-static.asp'>静态 DHCP/ARP</a> 列表以外的计算机将无法通过DHCP获取IP地址。</li>
					<li><b>最大活动DHCP租期</b> - 自己理解吧。</li>
					<li><b>静态租期时间</b> - 允许任何DHCP租用有效的绝对最大时间量。</li>
					<li><b>自定义配置</b> - 要添加到Dnsmasq配置文件的额外内容。</li>
				</ul>

				<i>DHCP 客户端 (WAN)：</i><br>
				<ul>
					<li><b>DHCPC 选项</b> -  DHCP客户端的额外选项。</li>
					<li><b>减少包大小</b> - 自己理解吧。</li>
				</ul>

				<i>其他相关注意事项/提示：</i><br>
				<ul>
					<li>文件/etc/dnsmasq.custom的内容也被添加到Dnsmasq配置文件的结尾（如果存在的话）。</li>
				</ul>
			</div>
		</div>
