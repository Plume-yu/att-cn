	...
	<script type="text/javascript">
		...
		function verifyFields(focused, quiet)
		{
			...
			if (s.value.search(/"rpc-enable":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-enable"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"peer-port":/) == 0)  {
				ferror.set(s, '无法设置 "peer-port"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"speed-limit-down-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "speed-limit-down-enabled"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"speed-limit-up-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "speed-limit-up-enabled"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"speed-limit-down":/) == 0)  {
				ferror.set(s, '无法设置 "speed-limit-down"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"speed-limit-up":/) == 0)  {
				ferror.set(s, '无法设置 "speed-limit-up"。 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"rpc-port":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-port" option here. 您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"rpc-whitelist-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-whitelist-enabled" 。白名单已禁用', quiet);
				ok = 0; }

			if (s.value.search(/"rpc-username":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-username"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"rpc-password":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-password"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"download-dir":/) == 0)  {
				ferror.set(s, '无法设置 "download-dir"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"incomplete-dir-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "incomplete-dir-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"incomplete-dir":/) == 0)  {
				ferror.set(s, '无法设置 "incomplete-dir" 。如果启用不完整目录，所有不完整的文件将被下载到“/download_dir/.incomplete”目录。', quiet);
				ok = 0; }

			if (s.value.search(/"peer-limit-global":/) == 0)  {
				ferror.set(s, '无法设置 "peer-limit-global"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"peer-limit-per-torrent":/) == 0)  {
				ferror.set(s, '无法设置 "peer-limit-per-torrent"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"upload-slots-per-torrent":/) == 0)  {
				ferror.set(s, '无法设置 "upload-slots-per-torrent"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"dht-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "dht-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"pex-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "pex-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"lpd-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "lpd-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"utp-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "utp-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"ratio-limit-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "ratio-limit-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"ratio-limit":/) == 0)  {
				ferror.set(s, '无法设置 "ratio-limit"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"rpc-authentication-required":/) == 0)  {
				ferror.set(s, '无法设置 "rpc-authentication-required"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"blocklist-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "blocklist-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"blocklist-url":/) == 0)  {
				ferror.set(s, '无法设置 "blocklist-url"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"download-queue-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "download-queue-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"download-queue-size":/) == 0)  {
				ferror.set(s, '无法设置 "download-queue-size"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"seed-queue-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "seed-queue-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"seed-queue-size":/) == 0)  {
				ferror.set(s, '无法设置 "seed-queue-size"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"idle-seeding-limit-enabled":/) == 0)  {
				ferror.set(s, '无法设置 "idle-seeding-limit-enabled"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"idle-seeding-limit":/) == 0)  {
				ferror.set(s, '无法设置 "idle-seeding-limit"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }

			if (s.value.search(/"message-level":/) == 0)  {
				ferror.set(s, '无法设置 "message-level"。您可以在Tomato界面中设置它', quiet);
				ok = 0; }
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="bit-basic-set">
			...
			<div class="content" id="torrent-config-section">
				...
				<script type="text/javascript">
					$('.clienttable').forms([
						{ title: '开启BT客户端', name: 'f_bt_enable', type: 'checkbox', value: nvram.bt_enable == '1', suffix: ' <small>*</small>' },
						{ title: '系统目录', multi: [
						...
						{ title: '进程守护', name: 'f_bt_check', type: 'checkbox', value: nvram.bt_check == '1', suffix: ' <small>*</small>' },
						{ title: '守护间隔', indent: 2, name: 'bt_check_time', type: 'text', maxlen: 5, size: 7, value: nvram.bt_check_time, suffix: ' <small>minutes (range: 1 - 55; default: 15)</small>' },
						{ title: '启动延迟', name: 'bt_sleep', type: 'text', maxlen: 5, size: 7, value: nvram.bt_sleep, suffix: ' <small>seconds (range: 1 - 60; default: 10)</small>' },
						...
						{ title: '使用 .incomplete/ 不完整目录', indent: 2, name: 'f_bt_incomplete', type: 'checkbox', value: nvram.bt_incomplete == '1' }
					]);
				</script>
				<h4>说明</h4>
				<ul>
					<li><b>启用BT客户端</b> - 警告！ - 如果你的路由器只有32MB的RAM，你必须使用交换。
					<li><b>系统目录</b> 包含进程守护等文件的目录的路径。
					<li><b>进程守护</b> - 如果启用，将以指定的间隔检查BT进程，并在崩溃后重新启动。
					<li><b>监听端口</b> - 用于BT客户端的端口。 确保此端口未使用。
				</ul>
			</div>
		</div>
		...
		<div class="box" data-box="bit-remote">
			...
			<div class="content">
				...
				<script type="text/javascript">
					$('.remoteacc').forms([
						{ title: '启用GUI界面', name: 'f_bt_rpc_enable', type: 'checkbox', value: nvram.bt_rpc_enable == '1' },
						{ title: '监听GUI端口', indent: 2, name: 'bt_port_gui', type: 'text', maxlen: 32, size: 5, value: nvram.bt_port_gui, suffix: ' <small>*</small>' },
				...
				<h4>说明</h4>
				<ul>
					<li><b>监听GUI端口</b> - 用于传输GUI的端口。 确保此端口未使用。
					<li><b>需要验证</b> - <b><i>强烈推荐</i></b>。启用后需要用户名/密码才能访问。
					<li><b>允许远程访问</b> - 此选项将从WAN侧打开传输GUI端口，并允许从Internet访问BT客户端。
				</ul>
			</div>
		</div>

		<div class="box" data-box="bit-limits">
			<div class="heading">限制</div>
			<div class="content">
				...
				<script type="text/javascript">
					$('.limitsgui').forms([
						...
						{ title: '全局连接数', name: 'bt_peer_limit_global', type: 'text', maxlen: 10, size: 7, value: nvram.bt_peer_limit_global, suffix: ' <small>(范围: 10 - 1000; 默认: 150)</small>' },
						{ title: '单种连接数', name: 'bt_peer_limit_per_torrent', type: 'text', maxlen: 10, size: 7, value: nvram.bt_peer_limit_per_torrent, suffix: ' <small>(范围: 1 - 200; 默认: 30)</small>' },
						{ title: '每种上传连接数', name: 'bt_ul_slot_per_torrent', type: 'text', maxlen: 10, size: 7, value: nvram.bt_ul_slot_per_torrent, suffix: ' <small>(范围: 1 - 50; 默认: 10)</small>' }
					]);
				</script>
			</div>
		</div>
		...
		<div class="box" data-box="bit-queue">
			...
			<div class="content">
				...
				<h4>说明</h4>
				<ul>
					<li><b>下载队列</b> - 如果启用，此选项将限制同时下载多少个文件。
					<li><b>做种队列</b> - 如果启用，此选项将限制同时上传多少个文件。
				</ul>
			</div>
		</div>
		...
		<div class="box" data-box="bit-advanced">
			...
			<script type="text/javascript">
				$('#tor-advancedsettings').forms([
					{ title: '查询更多可用资源', multi: [
						{ suffix: '启用DHT网络', name: 'f_bt_dht', type: 'checkbox', value: nvram.bt_dht == '1' },
						{ suffix: '启用PEX网络', name: 'f_bt_pex', type: 'checkbox', value: nvram.bt_pex == '1' },
						{ suffix: '启用LPD网络', name: 'f_bt_lpd', type: 'checkbox', value: nvram.bt_lpd == '1' },
						{ suffix: '启用uTP网络', name: 'f_bt_utp', type: 'checkbox', value: nvram.bt_utp == '1' }
					] },
					{ title: '消息级别', name: 'bt_message', type: 'select', options: [ ['0','无'], ['1','错误'], ['2','信息'], ['3','调试'] ], value: nvram.bt_message, suffix: ' ' },
					{ title: '保存设置位置', multi: [
						{ name: 'bt_settings', type: 'select', options: [
							...
							/* CIFS-END */
							['/tmp','内存 (临时)'], ['custom','自定义'] ], value: nvram.bt_settings, suffix: ' ' },
					...
					null,
					{ title: '<a href="https://trac.transmissionbt.com/wiki/EditConfigFiles" target="_new">Transmission</a><br>自定义设置', name: 'bt_custom', type: 'textarea', value: nvram.bt_custom, style: 'width: 100%; height: 80px;'  }
				]);
			</script>
		</div>
