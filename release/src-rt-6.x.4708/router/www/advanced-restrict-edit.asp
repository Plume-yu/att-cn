...
<content>
	...
	<script type="text/javascript">
		...
		var ipp2p = [
			[0,'IPP2P (禁用)'],[0xFFFF,'拦截全部IPP2P'],[1,'AppleJuice'],[2,'Ares'],[4,'BitTorrent'],[8,'Direct Connect'],
		...
		bpg.dataToView = function(data) {
			...
			s = '';
			if (data[5] != 0) s = ((data[5] == 1) ? '目标 ' : '自 ') + data[6] + ', ';
		...
		bpg.setup = function() {
			...
			this.init('res-bp-grid', 'sort', 500, [ { multi: [
				...
				{ type: 'select',
					options: [['a','任意端口'],['d','目标端口'],['s','源端口'],['x','源/目标端口']] },
				{ type: 'text', maxlen: 32 },
				{ type: 'select', options: ipp2p },
				{ type: 'select', options: layer7 },
				{ type: 'select',
					options: [[0,'任意地址'],[1,'目标IP'],[2,'源IP']] },
				{ type: 'text', maxlen: 64 }
			] } ] );
			this.headerSet(['规则']);
			this.showNewEditor();
			...
			<div class="content">
				...
				<script type="text/javascript">
					...
					$('#restriction').forms([
						...
						{ title: '端口/应用程序', indent: 2, text: '<table class="line-table col-sm-9" id="res-bp-grid"></table>' },
