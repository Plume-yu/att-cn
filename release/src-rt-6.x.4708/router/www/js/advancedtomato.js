...
function AdvancedTomato() {
	...
	if ( typeof nvram.at_update !== "undefined" && nvram.at_update != '' ) {
		...
		if ( n < lastUpdate || n == null ) {
			...
			$updateNotification = $( '<div class="alert alert-info icon"><a href="#" class="close" data-update="' + nvram.at_update.replace( '.', '' ) + '"><i class="icon-cancel"></i></a>\
				<h5>有可用更新!</h5>AdvancedTomato 版本 <b>' + nvram.at_update + '</b> 已发布，可下载。	&nbsp; <a target="_blank" href="https://advancedtomato.com/">请点击这里了解更多</a>。</div>' );
	...
	if ( typeof nvram.tomatoanon_answer !== "undefined" ) {
		...
		if ( nvram.tomatoanon_answer != '1' ) {
			...
			$( '.container' ).prepend( '<div class="alert alert-warning icon"><h5>注意</h5> 你还没有设置 <b>TomatoAnon</b>。\
				请到 <a onclick="loadPage(\'admin-tomatoanon.asp\')" href="#">TomatoAnon设置页面</a>做个选择。</div>' );
...
function systemUI() {
	...
	$.ajax(
		...
		var wanstatus = '<a title="请移步到状态概览" href="#" onclick="loadPage(\'#status-home.asp\');">' + ( ( stats.wanstatus[ 0 ] == 'Connected' ) ? '<span style="color: green;">已连接</span>' : "未连接") +  '</a>';
		$( '.system-ui .datasystem' ).html(
			...
			'<div class="desc">内存:</div><div class="value">' + stats.memory + '<div class="progress small"><div class="bar" style="width: ' + stats.memoryperc + '"></div></div></div>' +
	...
	$.ajax( { url: page, async: true, cache: false, timeout: 2950 } )
		...
		.fail( function( jqXHR, textStatus, errorThrown ) {
			...
			$( '.container .ajaxwrap' ).html( '<div class="box"><div class="heading">错误 - ' + jqXHR.status + '</div><div class="content">\
				<p>UI 无法与路由器通信!<br>这些问题通常发生在文件丢失,Web处理程序正忙或者连接丢失。</p>\
