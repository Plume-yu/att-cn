	...
	<script type="text/javascript">
		...
		th.verifyFields = function(row, quiet) {
			...
			if(E('_tinc_devicetype').value == 'tun'){
				...
				if (f[5].value != '') {
					ferror.set(f[5], "使用TAP接口类型时，子网留空。", quiet); return 0 ; }
				else {  ferror.clear(f[5]) }
			}

			if (E('_host_ed25519_key').value == "") {
				ferror.set(E('_host_ed25519_key'), "需要Ed25519公钥。", quiet); return 0 ; }
		...
		function verifyFields(focused, quiet)
		{
			...
			if (E('_tinc_name').value == "" && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_name'), "选中'连接互联网后运行'时需要使用主机名。", quiet); return 0 ; }
			else {  ferror.clear(E('_tinc_name')) }

			if (E('_tinc_private_ed25519').value == "" && E('_tinc_custom').value == "" && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_private_ed25519'), " 当选中'连接互联网后运行'时需要Ed25519私钥。 ", quiet); return 0 ; }
			...
			if (!hostdefined && E('_f_tinc_wanup').checked) {
				ferror.set(E('_tinc_name'), "主机名 \"" + E('_tinc_name').value + "\" 在选中 '连接互联网后运行' 时必须定义。", quiet); return 0 ; }
			else {  ferror.clear(E('_tinc_name')) };
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div id="vpn-tinc">
			<script type="text/javascript">
				...
				var action = ((tincup) ? 'title="停止"><i class="icon-stop"></i>' : 'title="运行"><i class="icon-play"></i>');
				var status = ((!tincup) ? '<small style="color: red">(未运行)</small>' : '<small style="color: green;">(运行中)</small>');

				html += '</ul>\
				<div class="box">\
				<div class="heading">Tinc 设置 <span id="version"></span> ' + status + '\
				...
				html += createFormFields([
					...
					{ title: '主机名', name: 'tinc_name', type: 'text', maxlen: 30, size: 25, value: nvram.tinc_name, suffix: ' <small>也必须在 \'主机\' 中定义。</small>' },
					{ title: '轮询间隔', name: 'tinc_poll', type: 'text', maxlen: 4, size: 5, value: nvram.tinc_poll, suffix: '&nbsp;<small>(以分钟为单位, 0 表示禁用)</small>' },	
				...
				html +='0（关闭），1（fast zlib）及最大9（best zlib），10（fast lzo）和11（best lzo）的整数。';
				...
				html +='<div style="visibility:hidden;text-align:right" id="generateWait">请稍候... <div class="spinner"></div></div>';
				html +='</div>';
				...
				html += '</div>'
				html += '<div style="visibility:hidden;text-align:right" id="statusWait">请稍候... <div class="spinner"></div></div>';
