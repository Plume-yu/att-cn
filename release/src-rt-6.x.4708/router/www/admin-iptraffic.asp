	...
	<script type="text/javascript">
		...
		function restoreButton() {
			...
			if ((name.length <= 3) || (name.substring(name.length - 3, name.length).toLowerCase() != '.gz')) {
				alert('错误的文件名， 正确的扩展名为 ".gz" 。');
				return false;
			}

			if (!confirm('是否从' + name + '恢复数据？')) return;
		...
		<div class="content">
			...
			<script type='text/javascript'>
				...
				$('#iptconfig').forms([
					...
					{ title: '创建新文件<br><small>(重设数据)</small>', indent: 2, name: 'f_new', type: 'checkbox', value: 0,
						suffix: ' &nbsp; <b id="newmsg" style="visibility:hidden"><small>如果是新文件则启用</small></b>' },
					{ title: '创建备份', indent: 2, name: 'f_bak', type: 'checkbox', value: nvram.cstats_bak == '1' },
					{ title: '每月第一天', name: 'cstats_offset', type: 'text', value: nvram.cstats_offset, maxlen: 2, size: 4 },
					...
					{ title: '启用自动发现', name: 'f_all', type: 'checkbox', value: nvram.cstats_all == '1', suffix: '&nbsp;<small>(在检测到流量时自动将新的IP加入监视列表)</small>' },
					{ title: '图形标签', name: 'cstats_labels', type: 'select', value: nvram.cstats_stime, options: [[0,'显示已知的主机名和IP'],[1,'优先显示主机名，无主机名则显示IP'],[2,'只显示IP']], value: nvram.cstats_labels }
					], { align: 'left' });
			</script>
			...
			<div class="row">
				...
				<div class="col-sm-12">
					<h4>说明</h4>
					<ul>
						<li>IP流量监测 通过路由器的<i>IPv4</i> 网络流量 。</li>
						<li>在开启这项功能之前请检查你的 <a class="ajaxload" href="basic-network.asp">LAN设置</a> 任何/所有的LAN接口必须至少16位的子网掩码（255.255.0.0）。</li>
						<li>不支持大子网（big subnets）监控。</li>
					</ul>
					...
					<ul>
						...
						<li>IIP流量监控跟踪数据/包，这将是要么 <i>从哪里来/离开</i> 或 <i>去/到达</i> 在LAN接口/子网的IP。</li>
