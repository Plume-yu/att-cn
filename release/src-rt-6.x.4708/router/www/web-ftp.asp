...
<content>
	<script type="text/javascript">
		...
		aftg.setup = function()
		{
			this.init('aft-grid', 'sort', 50, [
				...
				{ type: 'select', options: [['Read/Write', '读/写'],['Read Only', '只读'],['View Only', '仅查看'],['Private', '私有']] },
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="ftp-conf-main">
			<div class="heading">FTP 服务器设置</div>
			<div class="content">
				<div id="ftpconf"></div>
				<small style="color: red">注意：在为WAN启用FTP服务器时，请避免使用此选项。 它提供对路由器文件系统的完全访问！</small>
				<script type="text/javascript">
					$('#ftpconf').forms([
						...
						{ title: '允许远程<br>地址', indent: 2, name: 'f_ftp_sip', type: 'text', maxlen: 512, size: 64, value: nvram.ftp_sip,
							suffix: '<br><small>(可选; 如: "1.1.1.1", "1.1.1.0/24", "1.1.1.1 - 2.2.2.2" or "me.example.com")</small>' },
						{ title: '匿名用户权限', name: 'ftp_anonymous', type: 'select',
						...
						{ title: '记录FTP请求和响应', name: 'f_log_ftp', type: 'checkbox',
		...
		<div class="box" data-box="ftp-dirs">
			...
			<div class="content">
				...
				<small>
					*&nbsp;&nbsp;当没有指定目录时，/mnt用作根目录。
					<br>**&nbsp;在私人模式下，根目录是“私人根目录”下的目录，其名称与用户名称匹配。
				</small>
				...
				<script type="text/javascript">
					$('#ftpdirs').forms([
						...
						{ title: '公共根目录*', name: 'ftp_pubroot', type: 'text', maxlen: 256, size: 32,
							suffix: ' <small>(对于经过身份验证的用户访问，如果没有为用户指定目录的话)</small>',
		...
		<div class="box" data-box="ftp-limits">
			...
			<script type="text/javascript">
				$('#ftp-limits').forms([
					{ title: '允许登录的最大用户数', name: 'ftp_max', type: 'text', maxlen: 5, size: 7,
						suffix: ' <small>(0 -无限制)</small>',
						value: nvram.ftp_max },
					{ title: '来自相同IP的最大连接数', name: 'ftp_ipmax', type: 'text', maxlen: 5, size: 7,
						suffix: ' <small>(0 - 无限制)</small>',
					...
					{ title: '认证用户的最大带宽', name: 'ftp_rate', type: 'text', maxlen: 5, size: 7,
						suffix: ' <small>KBytes/sec (0 - 无限制)</small>',
					...
					{ title: '限制连接尝试', name: 'f_limit', type: 'checkbox',
						value: ftplimit[0] != 0 },
