...
<content>
	<script type="text/javascript">
		...
		function verifyFields(focused, quiet)
		{
			...
			if (!v_ip('_pptp_client_srvip', true) && !v_domain('_pptp_client_srvip', true)) { ferror.set(E('_pptp_client_srvip'), "服务器地址无效。", quiet); ret = 0; }
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" id="pptp-client">
			<div class="heading">PPTP客户端配置 <span class="pptp-client-status"></span></div>
			<div class="content"></div>
			<script type="text/javascript">
				$('#pptp-client .content').forms([
					{ title: '连接互联网后启动', name: 'f_pptp_client_enable', type: 'checkbox', value: nvram.pptp_client_enable != 0 },
					...
					{ title: '无状态MPPE连接', name: 'f_pptp_client_stateless', type: 'checkbox', value: nvram.pptp_client_stateless != 0 },
					{ title: '接受DNS配置', name: 'pptp_client_peerdns', type: 'select', options: [[0, '禁用'],[1, '是'],[2, '单独']], value: nvram.pptp_client_peerdns },
					{ title: '重定向到互联网流量', name: 'f_pptp_client_dfltroute', type: 'checkbox', value: nvram.pptp_client_dfltroute != 0 },
					{ title: '远程子网/网络掩码', multi: [
					...
					{ title: '在隧道上创建NAT', name: 'f_pptp_client_nat', type: 'checkbox', value: nvram.pptp_client_nat != 0 },
					{ title: 'MTU', multi: [
				...
				$('#pptp-client .pptp-client-status').after('<a href="#" data-toggle="tooltip" class="pull-right pptp-client-control" title="' +
					(pptpup ? '停止PPTP客户端' : '启动PPTP客户端') + '" onclick="toggle(\'pptpclient\', pptpup); return false;" id="_pptpclient_button">' + (pptpup ? '<i class="icon-stop"></i>' : '<i class="icon-play"></i>') + '</a>');
