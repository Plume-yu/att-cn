...
<content>
	<script type="text/javascript">
		...
		function restoreButton()
		{
			...
			if (!confirm('是否从 ' + name + '恢复数据?')) return false;
		...
		function save()
		{
			...
			if (en) {
				...
				if (((E('_rstats_stime').value * 1) <= 48) &&
					...
					if (!confirm('不建议对NVRAM或JFFS2频繁的存取，仍然继续?')) return;
				}
				if ((nvram.rstats_path != path) && (fom.rstats_path.value != path) && (path != '') && (path != '*nvram') &&
					(path.substr(path.length - 1, 1) != '/')) {
					if (!confirm('注意: ' + path + ' 将会被视为一个文件。 如果这是一个目录，请使用 /. 仍然继续？')) return;
				}
			...
			<script type="text/javascript">
				...
				$('#_fom').forms([
					...
					{ title: '保存频率', indent: 2, name: 'rstats_stime', type: 'select', value: nvram.rstats_stime, options: [
						...
						[9,'每9小时'],[12,'每12小时'],[24,'每24小时'],[48,'每2天'],[72,'每3天'],[96,'每4天'],
						[120,'每5天'],[144,'每6天'],[168,'每周']] },
					{ title: '关机时保存', indent: 2, name: 'f_sshut', type: 'checkbox', value: nvram.rstats_sshut == '1' },
					{ title: '创建新文件<br><small>(重置数据)</small>', indent: 2, name: 'f_new', type: 'checkbox', value: 0,
						suffix: ' &nbsp;<small id="newmsg" style="visibility:hidden">如果是新文件则启用</small>' },
					...
					{ title: '排除的接口', name: 'rstats_exclude', type: 'text', value: nvram.rstats_exclude, maxlen: 64, size: 50, suffix: '<small>(小写逗号分隔)</small>' }
					], { align: 'left' });
			</script>
