...
<content>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" id="dlna" data-box="dlna-media-serv">
			<div class="heading">Media / DLNA 服务器 </div>
			...
			<script type="text/javascript">
				...
				$('.content.mediadlna').forms([
					...
					{ title: '端口', name: 'ms_port', type: 'text', maxlen: 5, size: 6, value: nvram.ms_port, suffix: '<small>(范围: 0 - 65535; 默认:0 随机设置)</small>' },
					{ title: '数据库位置', multi: [
						{ name: 'f_loc', type: 'select', options: [['','内存 (临时)'],
							/* JFFS2-BEGIN */
							['/jffs/dlna','JFFS分区'],
							/* JFFS2-END */
							/* REMOVE-BEGIN */
					...
					{ title: '在启动时扫描*', indent: 2, name: 'f_ms_sas', type: 'checkbox', value: nvram.ms_sas == '1', hidden: 1 },
					{ title: '在下一次运行时重新扫描*', indent: 2, name: 'f_ms_rescan', type: 'checkbox', value: 0,
						suffix: '&nbsp; <small>(媒体扫描可能需要相当长的时间才能完成。)</small>' },
					{ title: 'TiVo支持', name: 'f_ms_tivo', type: 'checkbox', value: nvram.ms_tivo == '1' },
					{ title: '严格遵守DLNA标准', name: 'f_ms_stdlna', type: 'checkbox', value: nvram.ms_stdlna == '1' }
				]);

				$('#dlna .heading').append('<a href="#" class="minidlna-control pull-right" data-toggle="tooltip" title="' + (mdup ? '重新' : '') + '启动" onclick="restart(mdup)"' +
					'id="_restart_button">' + (mdup ? '<i class="icon-reboot"></i>' : '<i class="icon-play"></i>') + '</a>');
				$('#dlna .heading').append('<small ' + (mdup ? 'style="color: green;">(运行中' : 'style="color: red;">(已停止') + ')</small>');
			</script>
		</div>
