...
<div class="box">
	<div class="heading">查看日志 
		<a class="ajaxload pull-right" data-toggle="tooltip" title="日志设置" href="#admin-log.asp"><i class="icon-system"></i></a>
	</div>
	<div class="content">
		...
		<div id="logging">
			<div class="section">
				...
				<a href="logs/view.cgi?which=all&_http_id=<% nv(http_id) %>">查看全部</a><br /><br />
				<div class="input-append"><input class="span3" type="text" maxsize="32" id="find-text"> <button value="Find" onclick="find()" class="btn">查找 <i class="icon-search"></i></button></div>
				<i>在可用日志文件中搜索特定文本并显示匹配的行</i>
				<br><br /><hr>
				<a class="btn btn-primary" href="logs/syslog.txt?_http_id=<% nv(http_id) %>">下载日志文件 <i class="icon-download"></i></a>
			</div>
		</div>
