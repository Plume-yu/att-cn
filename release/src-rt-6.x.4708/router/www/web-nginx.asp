...
<content>
	<script type='text/javascript'>
		...
		function init()
		{
			...
			$('.nginx-status').after('<a href="#" data-toggle="tooltip" class="pull-right nginx-control" title="' +
				(nginxup ? '停止 NGINX 服务器' : '开启 NGINX 服务器') + '" onclick="toggle(\'nginxfp\', nginxup); return false;" id="_nginxfp_button">' + (nginxup ? '<i class="icon-stop"></i>' : '<i class="icon-play"></i>') + '</a>');
		}
	</script>
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class="box" data-box="nginx-webserver">
			<div class="heading">NGINX Web 服务器 <span class="nginx-status"></span></div>
		...
		<div class="box" data-box="nginx-usermanual">
			...
			<div class="content">
				<ul>
					<li><b> 状态按钮:</b> 快速启动 - 停止服务。 必须选中启用Web服务器才能修改设置。<br>
					<li><b> 开机启动:</b> 要激活Web服务器，请先保存。<br>
					<li><b> 保留配置文件:</b> 您是否手动修改了配置文件？ 勾选此框，将保留更改。<br>
					<li><b> 端口:</b> 要访问的Web服务器使用的端口。 当端口被其他服务使用时请检查冲突。<br>
					<li><b> 服务器名称:</b> 将出现在Internet浏览器顶部的名称。<br>
					<li><b> WWW根目录:</b> 上传网页文件的路径。<br>
					<li><b>如:<br></b>
					/tmp/mnt/HDD/www/ 你可以在USB路径中找到这个目录<br>
					<li><b>  NGINX自定义配置:</b> 您可以添加其他值到nginx.conf以满足您的需要。</li>
					<li>
						<b> 优先级:</b> 设置服务优先级高于在路由器上运行的其他进程。<br>
						操作系统内核具有优先级 -5.<br>
						不要选择比内核使用的值更低。 不要使用服务测试页来调整<br>
						服务器性能，它的性能低于外置存储，如<br>
						USB闪存盘，硬盘驱动器或SSD。<br>
					</li>
				</ul>
			</div>
	...
	<script type="text/javascript">
		$('.content.config-section').forms([
			...
			{ title: '启用PHP支持', name: 'f_nginx_php', type: 'checkbox', value: nvram.nginx_php == '1' },
			{ title: '启用 h5ai autoindex', name: 'f_nginx_ai', type: 'checkbox', value: nvram.nginx_ai == '1' },
			{ title: '运行方式', name: 'nginx_user', type: 'select',
			...
			{ title: '端口', name: 'nginx_port', type: 'text', maxlen: 5, size: 7, value: fixPort(nvram.nginx_port, 85), suffix: '<small> 默认: 85</small>' },
			{ title: '上传文件限制', name: 'nginx_upload', type: 'text', maxlen: 5, size: 7, value: nvram.nginx_upload, suffix: '<small> MB</small>'},
			{ title: '允许远程访问', name: 'f_nginx_remote', type: 'checkbox', value: nvram.nginx_remote == '1' },
			{ title: '服务器名称', name: 'nginx_fqdn', type: 'text', maxlen: 255, size: 20, value: nvram.nginx_fqdn },
			{ title: 'WWW根目录', name: 'nginx_docroot', type: 'text', maxlen: 255, size: 40, value: nvram.nginx_docroot, suffix: '<small>&nbsp;/index.html / index.htm / index.php</small>' },
			{ title: '优先级', name: 'nginx_priority', type: 'text', maxlen: 8, size:3, value: nvram.nginx_priority, suffix:'<small> 最大: -20, 最小: 19, 默认: 10</small>' }
		]);

		$('.content.config-adv').forms([
			{ title: 'HTTP Section - 自定义配置 (<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)', name: 'nginx_httpcustom', type: 'textarea', value: nvram.nginx_httpcustom, style: 'width: 100%; height: 140px;' },
			{ title: 'SERVER Section - 自定义配置 (<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)', name: 'nginx_servercustom', type: 'textarea', value: nvram.nginx_servercustom, style: 'width: 100%; height: 140px;'},
			{ title: '自定义配置 (<a href="http://wiki.nginx.org/Configuration" target="_new">NGINX<i class="icon-info"></i></a>)', name: 'nginx_custom', type: 'textarea', value: nvram.nginx_custom, style: 'width: 100%; height: 140px;' },
			{ title: '自定义配置 (<a href="http://php.net/manual/en/ini.php" target="_new">PHP<i class="icon-info"></i></a>)', name: 'nginx_phpconf', type: 'textarea', value: nvram.nginx_phpconf, style: 'width: 100%; height: 140px;' },
			{ title: '使用用户配置文件', name: 'f_nginx_override', type: 'checkbox', value: nvram.nginx_override == '1', suffix: '<small> 使用用户配置文件，一些GUI设置将被忽略</small>' },
