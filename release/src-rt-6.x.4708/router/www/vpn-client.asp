...
<content>
	...
	<script type="text/javascript">
		...
		RouteGrid.prototype.verifyFields = function(row, quiet)
		{
			...
			if (f[2].value.indexOf('>') >= 0 || f[2].value.indexOf('<') >= 0) { ferror.set(f[2], "值不能包含 '<' 和 '>' 。", quiet); ret = 0; }
			if (f[2].value.indexOf(' ') >= 0 || f[2].value.indexOf(',') >= 0) { ferror.set(f[2], "值不能包含 '空格' 和 ',' 每个条目只有一个IP或域。", quiet); ret = 0; }
			if (f[2].value.indexOf(' ') >= 0) { ferror.set(f[2], "值不能包含 '-'。 不支持的IP范围。", quiet); ret = 0; }
			return ret;
		}
		...
		function init()
		{
			...
			for (i = 0; i < tabs.length; ++i)
			{
				...
				routingTables[i].init('table_' + t + '_routing','sort', 0,[
					{ type: 'checkbox' },
					{ type: 'select', options: [[1, '从源IP'],[2, '到目标IP'],[3,'到域']] },
					{ type: 'text' }]);
		...
		<script type="text/javascript">
			...
			for (i = 0; i < tabs.length; ++i) {
				...
				htmlOut += createFormFields([
					...
					{ title: '桥接 TAP 至', indent: 2, name: 'vpn_'+t+'_br', type: 'select', options: [
						['br0','LAN (br0)*'],
						['br1','LAN1 (br1)'],
						['br2','LAN2 (br2)'],
						['br3','LAN3 (br3)']
				...
				htmlOut += '<div><ul><li><b>类型 -> 从源IP</b> - 例如: "1.2.3.4" or "1.2.3.0/24".'
				htmlOut += '<li><b>类型 -> 到目标IP</b> - 例如: "1.2.3.4" or "1.2.3.0/24".';
